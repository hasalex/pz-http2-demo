#! /bin/bash
docker_cmd="docker run --volume $(pwd)/ssl/server.crt:/ssl/server.crt --volume $(pwd)/ssl/server.key:/ssl/server.key --rm -it --ip 172.44.0.101 --name demo.proxy --network demo svagi/nghttp2"
